...
- **Video Colour Range**: Encoded video now emits standard limited-range BT.709 (`scale=in_range=pc:out_range=tv`), matching ComfyUI's own SaveVideo. The previous full-range output (`yuvj420p` + `color_range=pc`) avoided darkening, but `yuvj420p` is deprecated and players that normalise it to `yuv420p` re-expand the levels. The range conversion is now explicit, so no darkening occurs. *(`core/media_converter.py`)*
