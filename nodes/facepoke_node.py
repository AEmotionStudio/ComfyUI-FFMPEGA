...
def _resolve_video_path(video_path: str) -> str | None:
    ...
    Used by the ``/facepoke/*`` routes, which are unauthenticated — so a path
    is returned only when it lands inside ComfyUI's input/output/temp dirs (or
    FFMPEGA scratch). An arbitrary absolute path must not become a file the
    server reads frames out of and hands back to the caller.
    """
    if not video_path:
        return None
    ...
    if candidate:
        logger.warning(
            "[FacePoke] video_path %r resolved outside the allowed "
