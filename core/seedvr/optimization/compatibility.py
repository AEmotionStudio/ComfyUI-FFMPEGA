...
if not os.environ.get("SEEDVR2_OPTIMIZATIONS_LOGGED"):
    ...
    sage_status = "YES" if SAGE_ATTN_AVAILABLE else "NO"
    flash_status = "YES" if FLASH_ATTN_AVAILABLE else "NO"
    triton_status = "YES" if TRITON_AVAILABLE else "NO"
    ...
    if num_available == 3:
        print(f"[SeedVR2] Optimizations: SageAttention {sage_status} | Flash Attention {flash_status} | Triton {triton_status}")
    ...
    if NVIDIA_CONV3D_MEMORY_BUG_WORKAROUND:
        ...
        print(f"[SETUP] Conv3d workaround active: PyTorch {torch_ver}, cuDNN {cudnn_ver} (fixing VAE 3x memory bug)")
