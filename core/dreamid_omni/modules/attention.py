...
def flash_attention(
    ...
    if (version is None or version == 3) and FLASH_ATTN_3_AVAILABLE:
        ...
        # SageAttention varlen — same cu_seqlens API as flash_attn
        cu_seqlens_q = torch.cat([q_lens.new_zeros([1]), q_lens]).cumsum(
            0, dtype=torch.int32).to(q.device, non_blocking=True)
        cu_seqlens_k = torch.cat([k_lens.new_zeros([1]), k_lens]).cumsum(
            0, dtype=torch.int32).to(q.device, non_blocking=True)
        sm_scale = softmax_scale if softmax_scale is not None else (1.0 / (q.shape[-1] ** 0.5))
        x = sageattn_varlen(
            q, k, v,
        ...
        # Final fallback: PyTorch SDPA (always available)
        warnings.warn(
            'No Flash Attention or SageAttention available, using PyTorch SDPA. '
        ...
        # Reconstruct padded [B, L, N, C] tensors from packed varlen format
        q_padded = torch.zeros(b, lq, n_heads, head_dim, device=q.device, dtype=q.dtype)
        q_offset = 0
        for i in range(b):
            ql = q_lens[i].item()
        ...
        k_padded = torch.zeros(b, lk, n_heads, head_dim, device=k.device, dtype=k.dtype)
        v_padded = torch.zeros(b, lk, n_heads, head_dim, device=v.device, dtype=v.dtype)
        k_offset = 0
        for i in range(b):
            kl = k_lens[i].item()
            k_padded[i, :kl] = k[k_offset:k_offset + kl]
            v_padded[i, :kl] = v[k_offset:k_offset + kl]
            k_offset += kl
        ...
        attn_mask = torch.arange(lk, device=k.device).unsqueeze(0) < k_lens.unsqueeze(1)
