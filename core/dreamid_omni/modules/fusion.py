...
class FusionModel(nn.Module):
    ...
    def forward(
        ...
        _seq_offload = getattr(self, "sequential_offload", False)
        _offload_device = getattr(self, "_offload_device", "cpu")
        _compute_device = getattr(self, "_compute_device", vid[0].device if isinstance(vid, list) else vid.device)

        # --- Head: prepare kwargs (patch embed, timestep embed, etc.) ---
        if _seq_offload:
            # Move head components to GPU for prepare
            self.video_model.patch_embedding.to(_compute_device)
            self.video_model.text_embedding.to(_compute_device)
            self.video_model.time_embedding.to(_compute_device)
            self.video_model.time_projection.to(_compute_device)
            self.audio_model.patch_embedding.to(_compute_device)
            self.audio_model.text_embedding.to(_compute_device)
            self.audio_model.time_embedding.to(_compute_device)
            self.audio_model.time_projection.to(_compute_device)
        ...
        if _seq_offload:
            # Offload head components back to CPU
            self.video_model.patch_embedding.to(_offload_device)
            self.video_model.text_embedding.to(_offload_device)
            self.video_model.time_embedding.to(_offload_device)
            self.video_model.time_projection.to(_offload_device)
            self.audio_model.patch_embedding.to(_offload_device)
            self.audio_model.text_embedding.to(_offload_device)
            self.audio_model.time_embedding.to(_offload_device)
            self.audio_model.time_projection.to(_offload_device)
        ...
        for i in range(self.num_blocks):
            ...
            if _seq_offload:
                vid_block.to(_compute_device)
                audio_block.to(_compute_device)
            ...
            if _seq_offload:
                vid_block.to(_offload_device)
                audio_block.to(_offload_device)

        # --- Tail: unpatch, norm, head projection ---
        if _seq_offload:
            self.video_model.head.to(_compute_device)
            self.audio_model.head.to(_compute_device)
        ...
        if _seq_offload:
            self.video_model.head.to(_offload_device)
            self.audio_model.head.to(_offload_device)
            torch.cuda.empty_cache()
