...
"""Wan Video VAE — causal 3D autoencoder for video latent encoding.

Used by the SCAIL pipeline to encode reference images and pose videos
into latent space, and decode generated latents back to pixel space.
"""
...
class WanVAE_(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder3d(
            ...
            attn_scales, temperal_downsample[::-1], dropout,
        )
...
def _video_vae(pretrained_path=None, z_dim=None, device="cpu", **kwargs):
    ...
    with torch.device("meta"):
        model = WanVAE_(**cfg)
    logging.info("Loading VAE from %s", pretrained_path)
    if pretrained_path.endswith(".safetensors"):
        from safetensors.torch import load_file
        sd = load_file(pretrained_path, device=str(device))
    else:
...
class WanVAE:
    """Wan Video VAE wrapper with encode/decode for latent space ops."""

    def __init__(
        self,
        z_dim: int = 16,
        vae_pth: str = "Wan2.1_VAE.pth",
        dtype=torch.float,
        device="cuda",
    ):
        self.dtype = dtype
    ...
    def decode(self, zs: list[torch.Tensor]) -> list[torch.Tensor]:
        """Decode latents back to videos [C, T, H, W]."""
        with torch.amp.autocast('cuda', dtype=self.dtype):
            return [
