# coding: utf-8
# Vendored from kijai/ComfyUI-SCAIL-Pose/NLFPoseExtract/align3d.py
"""Camera parameter solving for 3D-to-2D pose alignment.

Uses scipy.optimize.minimize to find optimal camera intrinsic parameters
that align 3D NLF poses with 2D DWPose keypoints.
"""
...
def solve_new_camera_params_central(three_d_points, focal_length, imshape, new_2d_points):
    """Solve camera params minimising reprojection error (central weighting).

    Args:
        three_d_points: (N, 3) 3D joint positions.
        focal_length: Original camera focal length.
        imshape: (H, W) image dimensions.
        new_2d_points: (N, 2) target 2D positions.

    Returns:
        (K_final, m, s) — intrinsic matrix and scale factors.
    """
...
def solve_new_camera_params_down(three_d_points, focal_length, imshape, new_2d_points):
    """Solve camera params minimising reprojection error (lower-body weighting).

    Same interface as ``solve_new_camera_params_central`` but weights
    lower-body keypoints more heavily.
    """
