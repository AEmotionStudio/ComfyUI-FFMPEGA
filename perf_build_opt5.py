...
class Filter:
    ...
    def to_string_opt(self) -> str:
        ...
        if self.params:
            # Avoid the intermediate list kv_pairs if possible, or keep it but avoid str() if already str
            kv_pairs = []
            for k, v in self.params.items():
                if v is None:
                    kv_pairs.append(k)
                else:
                    # optimization: inline the type check and str call
                    if type(v) is str:
                        kv_pairs.append(f"{k}={sanitize_text_param(v)}")
                    else:
