...
class TestLightX2VLoRA:
    """Test LightX2V distill LoRA constants and integration."""

    def test_lora_hf_repo_defined(self):
        """_LORA_HF_REPO should point to lightx2v."""
        from core.kiwi_edit_synthesizer import _LORA_HF_REPO
        assert _LORA_HF_REPO == "lightx2v/Wan2.2-Distill-Loras"

    def test_lora_files_has_both_variants(self):
        """_LORA_FILES should have high_noise and low_noise."""
        from core.kiwi_edit_synthesizer import _LORA_FILES
        assert "high_noise" in _LORA_FILES
        assert "low_noise" in _LORA_FILES

    def test_lora_files_are_safetensors(self):
        """LoRA filenames should end with .safetensors."""
        from core.kiwi_edit_synthesizer import _LORA_FILES
        for variant, filename in _LORA_FILES.items():
            assert filename.endswith(".safetensors"), f"{variant}: {filename}"

    def test_lora_steps_is_4(self):
        """_LORA_STEPS should be 4 for distill LoRA."""
        from core.kiwi_edit_synthesizer import _LORA_STEPS
        assert _LORA_STEPS == 4

    def test_edit_video_accepts_lora_variant(self):
        """edit_video should accept lora_variant parameter."""
        import inspect
        from core.kiwi_edit_synthesizer import edit_video
        sig = inspect.signature(edit_video)
        assert "lora_variant" in sig.parameters

    def test_ensure_lora_downloaded_exists(self):
        """_ensure_lora_downloaded should be importable."""
        from core.kiwi_edit_synthesizer import _ensure_lora_downloaded
        assert callable(_ensure_lora_downloaded)

    def test_kiwi_lora_in_input_types(self):
        """kiwi_lora_enabled and kiwi_lora_variant should be in INPUT_TYPES."""
        pytest.importorskip("torch")
        from nodes.agent_node import FFMPEGAgentNode
        input_types = FFMPEGAgentNode.INPUT_TYPES()
        optional = input_types.get("optional", {})
        assert "kiwi_lora_enabled" in optional
        assert "kiwi_lora_variant" in optional

    def test_kiwi_lora_variant_choices(self):
        """kiwi_lora_variant should have high_noise and low_noise options."""
        pytest.importorskip("torch")
        from nodes.agent_node import FFMPEGAgentNode
        input_types = FFMPEGAgentNode.INPUT_TYPES()
        choices = input_types["optional"]["kiwi_lora_variant"][0]
        assert "high_noise" in choices
        assert "low_noise" in choices
