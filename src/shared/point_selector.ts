...
type EditorMode = "points" | "draw" | "box" | "nudge" | "smart";
 ...
 * @param framePath - optional on-disk path for SAM3/edge_map calls
 */
export function openPointSelector(
    ...
    // Frame path for SAM3 / edge_map calls
    ...
    `;
    exitBtn.onmouseenter = (): void => { exitBtn.style.background = "rgba(255,80,80,0.3)"; exitBtn.style.color = "#fff"; };
    exitBtn.onmouseleave = (): void => { exitBtn.style.background = "rgba(255,255,255,0.08)"; exitBtn.style.color = "#ccc"; };
    ...
    // SVG icon helpers
    const svgIcon = (d: string, size = 16): string =>
        `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${d}"/></svg>`;
    const ICO = {
    ...
    canvasViewport.style.cssText =
        "position:relative;width:90vw;height:75vh;overflow:hidden;border-radius:4px;" +
        "background:rgba(0,0,0,0.3);cursor:default;";
    ...
    const setZoom = (level: number, cx?: number, cy?: number): void => {
        const oldZoom = _zoom;
    ...
    const makeZoomBtn = (label: string, title: string): HTMLButtonElement => {
        ...
        b.onmouseenter = (): void => { b.style.background = "rgba(255,255,255,0.2)"; };
        b.onmouseleave = (): void => { b.style.background = "rgba(255,255,255,0.1)"; };
        return b;
    };
    ...
    zoomInBtn.onclick = (): void => { const vw = canvasViewport.clientWidth; const vh = canvasViewport.clientHeight; setZoom(_zoom * ZOOM_FACTOR, vw / 2, vh / 2); };
    zoomOutBtn.onclick = (): void => { const vw = canvasViewport.clientWidth; const vh = canvasViewport.clientHeight; setZoom(_zoom / ZOOM_FACTOR, vw / 2, vh / 2); };
    zoomFitBtn.onclick = (): void => fitToView();
    canvasViewport.addEventListener("wheel", (e: WheelEvent): void => {
        e.preventDefault();
    ...
    canvasViewport.addEventListener("pointerdown", (e: PointerEvent): void => {
        if (e.button === 1 || (e.button === 0 && e.altKey)) {
            e.preventDefault();
            _isPanning = true; _panStartX = e.clientX; _panStartY = e.clientY;
            _panStartPanX = _panX; _panStartPanY = _panY;
            canvasViewport.setPointerCapture(e.pointerId);
            canvasViewport.style.cursor = "grabbing";
        }
    });
    canvasViewport.addEventListener("pointermove", (e: PointerEvent): void => {
        if (!_isPanning) return;
        _panX = _panStartPanX + (e.clientX - _panStartX);
        _panY = _panStartPanY + (e.clientY - _panStartY);
        applyTransform();
    });
    canvasViewport.addEventListener("pointerup", (e: PointerEvent): void => {
        if (_isPanning) {
            _isPanning = false;
    ...
    settingsPanel.append(
        ...
        invertToggle.wrap, multiObjToggle.wrap, edgeRefineToggle.wrap,
        smartClampToggle.wrap, resetBtn,
    );
    overlay.appendChild(settingsPanel);

    // Rebuild point/box overlay with local expand/feather/invert applied to raw mask
    const rebuildPointOverlay = (): void => {
        if (!_rawMaskImg || !_rawMaskImg.complete) return;
        _pointOverlay = document.createElement("canvas");
        _pointOverlay.width = canvas.width; _pointOverlay.height = canvas.height;
        ...
        for (let i = 0; i < pd.data.length; i += 4) {
            ...
            if (showMask) {
                // Teal highlight for masked area
                pd.data[i] = 0; pd.data[i + 1] = 212; pd.data[i + 2] = 170;
                pd.data[i + 3] = featherVal > 0
                    ? Math.round(Math.min(150, ((invertVal ? 255 - val : val) / 255) * 150)) : 130;
            } else {
                // Strong darkening for unmasked area so the difference is clear
                pd.data[i] = 10; pd.data[i + 1] = 20; pd.data[i + 2] = 35; pd.data[i + 3] = 160;
            }
        }
    ...
    modeBtns.forEach(b => { b.style.padding = "6px 8px"; b.style.fontSize = "12px"; });
    ...
    // ── Mode colors and button mapping ──
    const _MODE_COLORS: Record<EditorMode, string> = {
        points: "#3a5a8a", draw: "#5a3a8a", box: "#8a5a3a", nudge: "#3a8a5a", smart: "#3a6a8a",
    };
    const _MODE_BTNS: [EditorMode, HTMLButtonElement][] = [
        ["points", modePointsBtn], ["draw", modeDrawBtn], ["box", modeBoxBtn],
        ["nudge", modeNudgeBtn], ["smart", modeSmartBtn],
    ];

    // Box prompt state
    let _boxDragging = false;
    let _boxStart: { x: number; y: number } | null = null;
    let _boxEnd: { x: number; y: number } | null = null;

    // Nudge state
    let _nudgeBaked = false;
    let _rawMaskImg: HTMLImageElement | null = null;

    // Smart draw state
    let _edgeMapData: ImageData | null = null;
    let _edgeMapLoading = false;
    let _edgeMapOff: HTMLCanvasElement | null = null;

    // SAM3 live mask overlay
    let _pointOverlay: HTMLCanvasElement | null = null;
    let _pointOverlayDirty = true;
    let _fetchTimer: ReturnType<typeof setTimeout> | null = null;

    const debouncedFetchMask = (): void => {
        ...
        _fetchTimer = setTimeout(() => {
            ...
            const body: Record<string, unknown> = { frame_path: _framePath, edge_refine: edgeRefineToggle.cb.checked };
            if (mode === "points") { body.points = pts; body.labels = lbls; body.multi_object = multiObjToggle.cb.checked; }
            else if (mode === "box") {
                const bs = _boxStart; const be = _boxEnd;
                if (bs && be) {
                    body.box = [
                        Math.round(Math.min(bs.x, be.x) * scaleX), Math.round(Math.min(bs.y, be.y) * scaleY),
                        Math.round(Math.max(bs.x, be.x) * scaleX), Math.round(Math.max(bs.y, be.y) * scaleY),
                    ];
                }
            }
            _sam3Loading = true; updateHeader();
            fetch("/ffmpega/sam3_point_mask", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
                .then(r => r.json())
                .then((data: { mask_b64?: string; raw_mask_b64?: string }) => {
                    if (!data.mask_b64) { _sam3Loading = false; updateHeader(); return; }
                    // Store raw B&W mask for nudge baking
                    if (data.raw_mask_b64) {
                        const rawImg = new Image();
                        rawImg.onload = (): void => { _rawMaskImg = rawImg; };
                        rawImg.src = "data:image/png;base64," + data.raw_mask_b64;
                    }
                    // Use server's pre-rendered colored overlay (with contours + multi-object colors)
                    const overlayImg = new Image();
                    overlayImg.onload = (): void => {
                        _sam3Loading = false; updateHeader();
                        _pointOverlay = document.createElement("canvas");
                        _pointOverlay.width = canvas.width; _pointOverlay.height = canvas.height;
                        const pCtx = _pointOverlay.getContext("2d");
                        if (pCtx) {
                            // The server overlay is a full RGB composite (original + colored masks + contours)
                            // Draw it at reduced opacity so it blends with our canvas image
                ...
                }).catch(() => { _sam3Loading = false; updateHeader(); });
        }, 300);
    };

    const fetchEdgeMap = (): void => {
        if (_edgeMapLoading || _edgeMapData) return;
        if (!_framePath) return;
        _edgeMapLoading = true;
        fetch("/ffmpega/edge_map", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ frame_path: _framePath }) })
            .then(r => r.json())
            .then((data: { edge_b64?: string; width?: number; height?: number }) => {
                ...
                img.onload = (): void => {
                    ...
                    if (eCtx) { eCtx.drawImage(img, 0, 0); _edgeMapData = eCtx.getImageData(0, 0, _edgeMapOff.width, _edgeMapOff.height); }
                    _edgeMapLoading = false; redraw();
                };
                img.src = "data:image/png;base64," + data.edge_b64;
            }).catch(() => { _edgeMapLoading = false; });
    };
    ...
    const updateModeUI = (): void => {
        ...
        if (mode === "nudge" && _rawMaskImg && _rawMaskImg.complete && !_nudgeBaked) {
            ...
            if (mCtx) { mCtx.drawImage(_rawMaskImg, 0, 0, maskOff.width, maskOff.height); _nudgeBaked = true; maskDirty = true; _greenOverlayDirty = true; }
        }
        updateHeader(); redraw();
    };
    _MODE_BTNS.forEach(([m, btn]) => { btn.onclick = (): void => { mode = m; updateModeUI(); }; });

    // ── Undo / Redo ──
    const _undoStack: ImageData[] = [];
    const _redoStack: ImageData[] = [];
    const MAX_UNDO = 30;
    const snapshotMask = (): void => {
        const mCtx = maskOff.getContext("2d");
    ...
    const undoMask = (): void => {
        ...
        maskDirty = true; _greenOverlayDirty = true; redraw();
    };
    const redoMask = (): void => {
        ...
        if (mode === "points" || mode === "box") {
            ...
            if (mode === "points") {
                for (let i = 0; i < pts.length; i++) {
                    ...
                    ctx.fillStyle = isPos ? "rgba(0,255,0,0.25)" : "rgba(255,0,0,0.25)"; ctx.fill();
                    ctx.strokeStyle = isPos ? "#0f0" : "#f00"; ctx.lineWidth = 2.5; ctx.stroke();
                    ctx.beginPath(); ctx.arc(px, py, 5, 0, Math.PI * 2);
                    ctx.fillStyle = isPos ? "#0f0" : "#f00"; ctx.fill();
                    ctx.font = "bold 16px sans-serif"; ctx.fillStyle = "#fff"; ctx.strokeStyle = "#000"; ctx.lineWidth = 3;
                ...
                const bs = _boxStart; const be = _boxEnd;
                if (bs && be) {
                    ctx.strokeStyle = "rgba(0,200,255,0.8)"; ctx.lineWidth = 2; ctx.setLineDash([6, 3]);
            ...
            // Draw/nudge/smart: green mask overlay
            if (_greenOverlayDirty || !_greenOverlay) {
                _greenOverlay = document.createElement("canvas");
                _greenOverlay.width = canvas.width; _greenOverlay.height = canvas.height;
                ...
                if (tmpCtx) {
                    ...
                    for (let i = 0; i < imgData.data.length; i += 4) {
                        ...
                        if (showGreen) {
                            imgData.data[i] = 0; imgData.data[i + 1] = 220; imgData.data[i + 2] = 80;
                            imgData.data[i + 3] = featherVal > 0
                                ? Math.round(Math.min(120, ((invertVal ? 255 - val : val) / 255) * 120)) : 100;
    ...
    // Smart brush helpers
    const paintOnMaskSmart = (canvasX: number, canvasY: number, erase: boolean): void => {
        if (!_edgeMapData) { paintOnMask(canvasX, canvasY, erase); return; }
        const mCtx = maskOff.getContext("2d");
        if (!mCtx) return;
        ...
        for (let py = 0; py < maskData.height; py++) {
            for (let px = 0; px < maskData.width; px++) {
                const imgX = ox + px; const imgY = oy + py;
                ...
                if (_smartClamp) {
                    ...
                    for (let s = 0; s <= steps; s++) {
                        ...
                        if (sx >= 0 && sx < edgeW && sy >= 0 && sy < _edgeMapData.height) {
                            if (_edgeMapData.data[(sy * edgeW + sx) * 4] > 128) { blocked = true; break; }
                        }
                    }
                    if (blocked) continue;
                } else {
                    if (imgX >= 0 && imgX < edgeW && imgY >= 0 && imgY < _edgeMapData.height) {
                        ...
                        if (edgeVal > 128) { const idx = (py * maskData.width + px) * 4; maskData.data[idx] = maskData.data[idx+1] = maskData.data[idx+2] = fillVal; maskData.data[idx+3] = 255; continue; }
                    }
                }
        ...
        maskDirty = true; _greenOverlayDirty = true;
    };
    const paintLineSmart = (x1: number, y1: number, x2: number, y2: number, erase: boolean): void => {
        const dist = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
        const steps = Math.max(1, Math.floor(dist / 3));
        for (let i = 0; i <= steps; i++) { const t = i / steps; paintOnMaskSmart(x1 + (x2 - x1) * t, y1 + (y2 - y1) * t, erase); }
    };

    canvas.addEventListener("mousedown", (e: MouseEvent): void => {
        if (mode === "draw" || mode === "nudge" || mode === "smart") {
            e.preventDefault();
            // Nudge: bake SAM3 mask first
            if (mode === "nudge" && !_nudgeBaked && _rawMaskImg && _rawMaskImg.complete) {
                snapshotMask();
                const mCtx = maskOff.getContext("2d");
                if (mCtx) { mCtx.drawImage(_rawMaskImg, 0, 0, maskOff.width, maskOff.height); _nudgeBaked = true; maskDirty = true; }
            }
            snapshotMask();
            isDrawing = true; drawButton = e.button;
            const pos = getCanvasPos(e); lastDrawX = pos.x; lastDrawY = pos.y;
            ...
            e.preventDefault();
            _boxDragging = true;
            _boxStart = getCanvasPos(e); _boxEnd = _boxStart;
            redraw();
        }
    });
    ...
    canvas.addEventListener("mousemove", (e: MouseEvent): void => {
        if (mode === "box" && _boxDragging) {
            _boxEnd = getCanvasPos(e); redraw(); return;
        }
        if (mode === "draw" || mode === "nudge" || mode === "smart") {
            ...
            if (ctx) {
                ...
                ctx.beginPath(); ctx.arc(pos.x, pos.y, brushR, 0, Math.PI * 2);
    ...
    const stopDrawing = (): void => {
        if (isDrawing) { _greenOverlayDirty = true; redraw(); }
        isDrawing = false; drawButton = -1; lastDrawX = -1; lastDrawY = -1;
    };
    ...
    canvas.addEventListener("mouseleave", (): void => {
        if (mode === "box") { _boxDragging = false; }
        stopDrawing();
    ...
    canvas.addEventListener("click", (e: MouseEvent): void => {
        ...
        if (hitIdx >= 0) { pts.splice(hitIdx, 1); lbls.splice(hitIdx, 1); }
        else { pts.push([Math.round(pos.x * scaleX), Math.round(pos.y * scaleY)]); lbls.push(1); }
        redraw(); debouncedFetchMask();
    });
    canvas.addEventListener("contextmenu", (e: MouseEvent): void => {
        ...
        const hitIdx = findNearPoint(pos.x, pos.y);
        if (hitIdx >= 0) { pts.splice(hitIdx, 1); lbls.splice(hitIdx, 1); }
        else { pts.push([Math.round(pos.x * scaleX), Math.round(pos.y * scaleY)]); lbls.push(0); }
        redraw(); debouncedFetchMask();
    });

    // Removed click-outside-to-close — too easy to accidentally trigger while painting
    overlay.addEventListener("contextmenu", (e: Event): void => e.preventDefault());

    // ── Buttons ──
    clearBtn.onclick = (): void => {
        if (mode === "points" || mode === "box") {
            pts.length = 0; lbls.length = 0;
            _boxStart = null; _boxEnd = null;
            _pointOverlay = null; _pointOverlayDirty = true;
            _rawMaskImg = null;
        } else {
            const mCtx = maskOff.getContext("2d");
            if (mCtx) { mCtx.fillStyle = "#000"; mCtx.fillRect(0, 0, maskOff.width, maskOff.height); }
            maskDirty = false; _greenOverlayDirty = true;
            _nudgeBaked = false;
            if (mode === "smart") { _edgeMapData = null; _edgeMapOff = null; _edgeMapLoading = false; }
        }
    ...
    applyBtn.onclick = (): void => {
        ...
        if (mpWidget) { mpWidget.value = data; }
        else {
            const w = node.addWidget("text", "mask_points_data", data, () => { /* no-op */ }, { serialize: true });
            w.type = "text";
    ...
    const keyHandler = (e: KeyboardEvent): void => {
        ...
        if (e.key === "z" && (e.ctrlKey || e.metaKey) && !e.shiftKey) { e.preventDefault(); undoMask(); }
        if ((e.key === "z" && (e.ctrlKey || e.metaKey) && e.shiftKey) || (e.key === "y" && (e.ctrlKey || e.metaKey))) { e.preventDefault(); redoMask(); }
        if (e.key === "f" && !e.ctrlKey && !e.metaKey && !e.shiftKey) { e.preventDefault(); fitToView(); }
    };
